...
; IIGS monitor.
        ...
        .import   read_line
        .import   arg, error_msg, instr, instr_len, operand, operand_size, operand_type, ibuff, IBUFFSZ, token
        .import   print_instruction
...
:       sta     arg,y
        ...
        bne     @copy
@find:  longm
        ldaw    #.loword(instr_mnemonics)
        sta     ptr
        ldaw    #.hiword(instr_mnemonics)
        sta     ptr + 2
        ldxw    #0                  ; entry counter
        ldyw    #2
@entry: lda     [ptr]
        cmp     arg
        bne     @next
...
:       lda       operand
        ...
        bge       @pcrl
        lda       operand
        andw      #$FF80        ; mask upper 9 bits
        beq       :+            ; must be all zeroe
        cmpw      #$FF80        ; or all ones
        beq       :+
        sec
        rts
:       ldaw      #Operand::pcr
